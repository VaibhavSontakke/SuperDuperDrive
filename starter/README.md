...
1. Managing userDetails access with Spring Security
 ...
 - You also need to implement a custom `AuthenticationProvider` which authorizes userDetails logins by matching their credentials against those stored in the database.  
...
2. Handling front-end calls with controllers
 ...
 - The controllers you write should also be responsible for determining what, if any, error messages the application displays to the userDetails. When a controller processes front-end requests, it should delegate the individual steps and logic of those requests to other services in the application, but it should interpret the results to ensure a smooth userDetails experience.
...
2. Sign Up page
 ...
 - Remember to store the userDetails's password securely!


3. Home page
The home page is the center of the application and hosts the three required pieces of functionality. The existing template presents them as three tabs that can be clicked through by the userDetails:


 i. Files
  - The userDetails should be able to upload files and see any files they previously uploaded. 

  - The userDetails should be able to view/download or delete previously-uploaded files.
  - Any errors related to fileDetail actions should be displayed. For example, a userDetails should not be able to upload two files with the same name, but they'll never know unless you tell them!


 ii. Notes
  - The userDetails should be able to create notes and see a list of the notes they have previously created.
  - The userDetails should be able to edit or delete previously-created notes.

 iii. Credentials
 - The userDetails should be able to store credentials for specific websites and see a list of the credentials they've previously stored. If you display passwords in this list, make sure they're encrypted!
 - The userDetails should be able to view/edit or delete individual credentials. When the userDetails views the userCredentialDetails, they should be able to see the unencrypted password.

The home page should have a logout button that allows the userDetails to logout of the application and keep their data private.

### Testing
Your tech lead trusts you to do a good job, but testing is important whether you're an excel number-cruncher or a full-stack coding superstar! The QA team at Super*Duper*Drive carries out extensive userDetails testing. Still, your tech lead wants you to write some simple Selenium tests to verify userDetails-facing functionality and prove that your code is feature-complete before the testers get their hands on it.

1. Write tests for userDetails signup, login, and unauthorized access restrictions.
 - Write a test that verifies that an unauthorized userDetails can only access the login and signup pages.
 - Write a test that signs up a new userDetails, logs in, verifies that the home page is accessible, logs out, and verifies that the home page is no longer accessible. 


2. Write tests for noteDetail creation, viewing, editing, and deletion.
 - Write a test that creates a noteDetail, and verifies it is displayed.
 - Write a test that edits an existing noteDetail and verifies that the changes are displayed.
 - Write a test that deletes a noteDetail and verifies that the noteDetail is no longer displayed.


3. Write tests for userCredentialDetails creation, viewing, editing, and deletion.
...
### Password Security
Make sure not to save the plain text credentials of the application's users in the database. That's a recipe for data breach disaster! Use a hashing function to store a scrambled version instead. Your tech lead gave you a class called `HashService` that can hash passwords for you. When the userDetails signs up, you only store a hashed version of their password in the database, and on login, you hash the password attempt before comparing it with the hashed password in the database. Your tech lead knows that can be a little confusing, so they provided this code sample to help illustrate the idea:
...
For storing credentials in the main part of the application, we can't hash passwords because it's a one-way operation. The userDetails needs access to the unhashed password, after all! So instead, you should encrypt the passwords. Your tech lead provided you with a class called `EncryptionService` that can encrypt and decrypt passwords. When a userDetails adds new credentials, encrypt the password before storing it in the database. When the userDetails views those credentials, decrypt the password before displaying it. Here's a little code snippet on how to use `EncryptionService`:
